...
const ArticleCard = ({ title, excerpt, category, readTime, date, featured = false, slug }: ArticleCardProps) => {
  return (
    <Link to={`/article/${slug}`}>
      <article
        className={`group relative rounded-xl border border-border bg-card overflow-hidden transition-all duration-300 hover:border-primary/50 hover:shadow-[var(--shadow-card-hover)] h-full ${
